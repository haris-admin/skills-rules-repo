...
description: Decomposition playbook + anti-temptation rules for an orchestrator profile routing work through Kanban. The "don't do the work yourself" rule and the basic lifecycle are auto-injected into every kanban worker's system prompt; this skill is the deeper playbook when you're specifically playing the orchestrator role. Use when decomposing a request into Kanban tasks, deciding whether work belongs on the board, fanning out parallel or dependent cards to specialist profiles, setting up goal-mode cards, or recovering stuck/blocked workers.
version: 3.0.0

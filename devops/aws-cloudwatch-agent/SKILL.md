...
When running database queries through SSM send-command (e.g., psql on backend EC2 to query a private RDS), SSM's 3000-char response truncation means you must batch queries and use base64 encoding for SQL files — or switch to SSM port forwarding for large result sets. See [SSM SQL Query Patterns](references/ssm-sql-query-patterns.md) for the base64 SQL file pattern, labeled multi-query output parsing, port forwarding, and four hard-won pitfalls (the `AT TIME ZONE` string-cast trap, never hardcoding the RDS username, JSON-quoting SQL with newlines, and the cents/dollars double-division bug).
...
When the CloudWatch agent is running but not shipping logs — or logs went silent days/weeks ago — the local log files on the EC2 instance are **still there**. Use SSM Run Command to grep them directly (8-15s per command, then poll with `get-command-invocation`). This is a critical fallback when CloudWatch shows 28+ days of silence but the application is definitely running. See [SSM Log Fallback Diagnostics](references/ssm-log-fallback-diagnostics.md) for the full command patterns, the grep-signal table (app alive, auth failures, SES success/failure), noise filtering, and how to confirm whether CloudWatch going silent was just a restart gap.
...
- See `references/bedrock-model-access-diagnosis.md` when a Bedrock model call fails — diagnosing model-access/region-enablement errors
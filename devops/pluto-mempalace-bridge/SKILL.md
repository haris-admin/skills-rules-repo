...
**Cross-domain pairs:** 10 predefined chamber pairs (e.g., `regulatory-ai × fintech-aml`, `agentic-security × agent-architecture`) that are semantically analyzed for shared concepts, drawn from **16 registered chambers**. First run (May 27): 108 docs → 9 patterns + 2 contradictions. See `references/synthesis-example-2026-05-27.md` for the full output from the inaugural run.

**Known limits & 16 Sep 2026 fixes** (verified against the live palace):
- **"16 chambers queried" ≠ 16 chambers with data.** The engine loops all 16 names, but on 16 Sep only **8 returned in-window docs**. 5 chambers have never been populated (`data-sovereignty`, `regtech-tools`, `insurtech`, `agent-observability`, `voice-creative` = 0 docs total); `agentic-security` (37 total), `digital-identity` (2), `legal-professional` (9) hold docs but none inside the 3-day window. Empty ≠ broken, but don't read the chamber count as coverage.
- **Doc count is CHUNK count, not source count.** On 16 Sep the headline said 193 docs but that was **38 distinct sources** — the Genspark morning brief alone is chunked into 79 docs. The report now prints both (`distinct_sources_analyzed` + per-chamber `distinct_sources`); a single fat source can dominate every signal.
- **Contradiction labels were hardcoded.** Every detected pair printed "25 pro-regulation vs 17 anti-regulation" regardless of the pattern that matched (surge/decline and Australia/offshore were both mislabeled; bug visible in the 13–14 Sep outputs). `CONTRADICTION_PATTERNS` now carries a per-pair label, and each contradiction reports per-chamber counts + verbatim evidence snippets.
- **`global`/`foreign`/`import` were false-positive magnets** on the onshore/offshore side — the bare word `global` matched newsletter titles ("Global Fintech News"), inflating that side 70× → 5× once tightened to real offshoring language. If a contradiction count looks huge, check the evidence snippets before acting on it.
...
- ~~`Vercel Monitor — AML Hive`~~ — **RETIRED (Aug 8, 2026)**; Vercel no longer used, the cron (`745ec76c6bf9`) is gone. AML Hive deployment health is now covered by the 4×/day `amlhive_prod_monitor.py` fleet monitor (public health endpoints + EC2/Docker/CloudWatch).
...
18. ~~**Vercel Monitor (AM/PM)**~~ — RETIRED Aug 2026 (Vercel no longer used). Superseded by the AMLHive fleet monitor.
...
- **🔴 chromadb import fails with opentelemetry version drift (fixed Aug 15, 2026).** `import chromadb` fails with `ModuleNotFoundError: No module named 'opentelemetry.exporter.otlp.proto.common._exporter_metrics'`. Root cause: mixed opentelemetry package versions in `~/.hermes/repo/venv` (grpc 1.44.0 vs common/api/sdk 1.39.1). When this happens, the Chamber Refresh cron silently feeds 0 items — every feed shows `Traceback ... pluto_mempalace_feeder.py` and the footer reads `Fed 0 new item(s)`. **Fix (align all opentelemetry to one version):**
  ...
  **🔴 RECURRED Aug 22, 2026:** The exact same drift re-appeared (`opentelemetry-exporter-otlp-proto-grpc` at 1.44.0 while api/proto/sdk/common/http pinned at 1.39.1), silently breaking the Chamber Refresh for 4 days (Aug 18–21 all showed `Fed 0 new item(s)` with a feeder Traceback, masked by `last_status: ok`). Root cause: `hermes update` (Mon/Fri 4AM) reinstalls core deps and re-introduces the version mismatch. **🔴 RECURRED Sep 11, 2026 (3rd time)** — same split (grpc 1.44.0 alone); `hermes update` ran 03:03 that morning. The Chamber Refresh smoke-test added after recurrence 2 worked as designed: the cron exited 1 with the loud `🔴 CRITICAL: chromadb import failed` banner instead of silently feeding 0. 
 **DURABLE FIX (in place since 11 Sep 2026):** `~/.hermes/scripts/fix_opentelemetry_pins.sh` — idempotent; no-ops when `import chromadb` already succeeds, otherwise reinstalls the six pinned 1.44.0 packages and re-verifies. It is invoked automatically at the end of `hermes_update_check.sh` right after any successful `hermes update`, so the drift is repaired in the same window that introduces it. **Added 12 Sep 2026 (belt and braces):** `pluto_chamber_refresh.py` now SELF-HEALS — when its pre-feed smoke test fails it runs `fix_opentelemetry_pins.sh` inline, re-tests `import chromadb`, and only exits 1 if the import is still broken. So a drift that slips past the update hook is repaired inside the 05:25 feed instead of leaving the chambers dark.
  **🔴 RECURRED Sep 25, 2026 (4th time) — the hook was firing but repairing the WRONG VENV.** The 03:06 `hermes update` (v0.21.3→v0.21.5) re-introduced the drift; the 04:55/05:00 watcher feeds then silently lost 100 findings (see the watcher pitfall below). Root cause of the non-repair: `fix_opentelemetry_pins.sh` hardcoded `VENV=/home/habib/.hermes/repo/venv`, which **does not have chromadb installed at all**, while the pipeline feeds with `~/.hermes/venv/bin/python3` (`mempalace_watcher.py` → `VENV_PYTHON`, and `pluto_chamber_refresh.py`). The script therefore "repaired" an env that could never work and never touched the broken one. **Fixed:** it now loops over every candidate venv and repairs only those where chromadb is actually installed. **Rule: any venv-pinning repair script must target the interpreter the pipeline really uses — verify with `grep -n VENV_PYTHON ~/.hermes/scripts/mempalace_watcher.py`, not from memory.**
  **⚠️ Never smoke-test with a bare `import chromadb`.** A stray `chromadb/` DIRECTORY on the path (e.g. `~/.hermes/chromadb/`, an 188KB `chroma.sqlite3` with 0 collections) is importable as a NAMESPACE package: the import "succeeds" while `chromadb.PersistentClient` does not exist and `chromadb.__version__` raises `AttributeError`. A bare-import health check therefore reports a broken venv as healthy and skips the repair — and run from `~/.hermes` it can also mask a real failure. **Always assert a real attribute: `python -c 'import chromadb; chromadb.PersistentClient'`,** and detect "is chromadb installed here?" with `importlib.util.find_spec('chromadb')` requiring `spec.origin` (namespace portions have `origin=None`).
...
- **🔴 THE WATCHER CAN REPORT SUCCESS WHILE FEEDING NOTHING — `fed` is the PARSED count, not the STORED count (fixed 2026-09-25).** `mempalace_watcher.py`'s `process_file()` set `result['fed'] = feed_result['findings_count']` (i.e. `len(parsed findings)`) and then **unconditionally** wrote the `.processed/<stem>.done` marker with `status: 'processed'` — it never inspected the feeder's own report or its exit code. When the feeder could not import chromadb (the venv drift above), the watcher still logged `"fed": 83, "status": "processed"` and marked the file done, so the content was **permanently unrecoverable without manual intervention**. On 2026-09-25 this silently lost the day's Genspark briefing (83 findings) and Claude Daily Research (17 findings) — the chamber held only 6 docs for the date (the research feed) against 138 the previous day. **Recovery recipe** (the only way to get such files back): delete the marker (`rm mempalace-inputs/.processed/<stem>.done`) and re-run `mempalace_watcher.py`; the file is re-parsed and re-fed. **Fix applied:** `feed_to_mempalace()` now parses the feeder's stdout and requires `returncode == 0` AND `status == 'stored'` AND `findings_stored > 0`, returning `ok`/`stored`/`error`; `process_file()` writes the `.done` marker ONLY on `ok` and otherwise reports `status: 'failed'` with `fed: 0` **and no marker**, so the 5-minute cron retries automatically. `ensure_chromadb()` (smoke test + `fix_opentelemetry_pins.sh` self-heal) now gates `process_all()` and `--file`.
  **Detection rule (run after any feed-related incident, or when the day's doc count looks low):** count today's stored ids and compare against what the watcher claimed, e.g. `python -c "import chromadb,re,collections; c=chromadb.PersistentClient(path='/mnt/c/Users/habib/.mempalace/palace'); print(collections.Counter(re.search(r'(20\d{6})_(\d{4})',i).group(1) for col in [c.get_collection(x.name) for x in c.list_collections()] for i in col.get(include=[])['ids'] if re.search(r'(20\d{6})_(\d{4})',i)))"` — every research/watcher feed gets ids of the form `pluto_<YYYYMMDD>_<HHMMSS>_n`, so the day's total must equal the sum of the day's parsed counts. A day that is short by ~100 docs is this bug. Never accept the watcher's `fed` at face value.
...
- **🔴 A bare `python3` is NOT safe in cron — resolve an interpreter that can actually import chromadb (2026-09-25).** The Hermes runtime venv (`/home/habib/.hermes/repo/venv/bin`) sits **first** on PATH in gateway-spawned processes but has **no chromadb installed**, so `subprocess.run(['python3', ...])` inside a cron script silently picks the wrong interpreter. The 05:25 Chamber Refresh died on exactly this (`ModuleNotFoundError: No module named 'chromadb'` in its smoke test) while every venv that actually feeds the chambers was healthy — and the inline self-heal could not repair it, because `fix_opentelemetry_pins.sh` only repairs venvs where chromadb is *already* installed, i.e. it never touched the interpreter the smoke test was using. **Rule: never invoke `python3` by name for anything that touches chromadb.** `pluto_chamber_refresh.py` now probes candidates in order (`$PLUTO_CHAMBER_PYTHON` → `~/.hermes/venv/bin/python3` → `/usr/bin/python3` → `sys.executable` → `python3`) and uses the first that passes `import chromadb`, for BOTH the smoke test and every `run_python()` feeder call. When debugging any "chromadb missing" cron failure, compare `which -a python3` in the cron environment against the venv that actually has it — never assume the interpreter. Verify a fix by running the script directly with the WRONG interpreter on purpose: it must still resolve a working one and exit 0.
- **✅ `result:null` in the palace WAL log is NOT a failure indicator (corrected 2026-09-25).** `mempalace_health.py` treated every new `result:null` journal entry as "the write path is failing silently again" — a rule inherited from the Aug 2026 incident, when it genuinely did mean that. Verified live 2026-09-25: the journal held 4 new `result:null` entries AND every corresponding triple (AUSTRAC → Western Union Australia, ASIC INFO 225, Australia FATF APG evaluation) was present in `knowledge_graph.sqlite3` with `integrity_check = ok` and 164 rows total. The null is simply the write helpers' normal return value. The check now reconciles BY EFFECT: it copies the KG trio to ext4, asserts it opens and passes `integrity_check`, and alerts only on a real row regression (entities/triples count dropping) or an unreadable graph. **Lesson: alert on the effect, never on an inferred return value.**
- **🔴 WAL-mode SQLite on `/mnt/c` is unreadable from WSL — copy it to ext4 first.** `sqlite3.connect('/mnt/c/Users/habib/.mempalace/knowledge_graph.sqlite3')` raises `disk I/O error` (for `mode=ro` and `rw` alike) because drvfs cannot mmap the `-shm` sidecar — even when the DB is perfectly healthy and `integrity_check` returns `ok` once the trio (`db` + `-wal` + `-shm`) is copied to an ext4 path. The palace's `chroma.sqlite3` does not use WAL, which is why collection reads work in place while the graph does not. Any WSL-side reader of the KG must copy first — `mempalace_health.py::reconcile_kg()` does exactly this, into `~/.hermes/cache/scratch/mempalace_kg/`. A multi-MB `-wal` next to the graph is therefore normal, not "a stuck WAL".

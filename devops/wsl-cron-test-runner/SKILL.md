...
## Harness environments — a subset dependency install is the #1 cause of "the suite is broken"

When a runner builds its own throwaway venv and pip-installs a hand-picked list (pytest only, or pytest + psycopg2 + alembic) instead of the repo's real requirements, pytest exits **4** and the summary prints `0 passed, 0 failed, 0 errors / 0 total`. Exit code 4 is pytest's USAGE/collection-fatal code, not a test verdict; the real error sits one line up:

```
ImportError while loading conftest '.../backend/tests/conftest.py'
  .../conftest.py:13: in <module>   import httpx
E ModuleNotFoundError: No module named 'httpx'
```

Rules:
- **`0 passed, 0 failed` is NEVER a pass, and a pytest exit code in {2,3,4,5} means the suite never ran.** Report both as an ENVIRONMENT BLOCKER, kept distinct from "tests failed", so the alert says which of the two happened instead of collapsing into one red.
- **Install from the repo's LOCK, not `requirements*.txt`.** A hand-maintained requirements file is the same trap one level up: it is a subset, and pip silently SKIPS every requirement whose environment marker does not match the running interpreter — so a suite on the wrong Python reports hundreds of `ModuleNotFoundError` collection errors that read like a broken product. Sync the lock (`uv sync --locked`, or `poetry export --without-hashes` → `pip install -r`) on the interpreter `pyproject.toml` declares; `poetry install` states the mismatch outright: `Current Python version (3.11.x) is not allowed by the project (^3.13.12)`.
- **A main-deps lock export does NOT contain the dev group** — `pytest` itself can be absent from it, and the test-requirements file normally re-pins runtime deps (it opens with a `-r requirements.txt` include). Strip that include line before layering the test file onto a lock install, or the second install downgrades what the lock just resolved.
- **Set `cwd` to the directory whose config applies** (repo root, or `backend/`) so `pyproject.toml`/`pytest.ini` is honoured (asyncio_mode, testpaths, markers). An absolute tests path with no `cwd` silently ignores the project's pytest config.
- **Surface the first ~15 lines of pytest output in the report.** A bare `returncode 4` tells a reader nothing; the ImportError tells them everything.
- **Check for a Windows-flavoured venv before trusting one.** A `.venv` inside a `/mnt/c` checkout may have been created by Windows Python: `bin/` holds only activate scripts and executing `.venv/bin/pytest` fails with `No such file or directory` (the shebang points at a Windows path; the real interpreter is `Scripts/python.exe`). `test -x .venv/bin/python` is the tell — when it fails, the "repo's own venv" fallback is a dead end and you must build a Linux venv.
- **Never put test venvs in `/tmp`.** They are wiped on every reboot, so each run pays a cold install and can hit the runner's pip timeout (both venvs were gone after a reboot). Use a persistent path — `~/.hermes/cache/testenvs/<repo-slug>` — reused across runs, with the pip timeout raised so a cold build cannot be killed mid-install.

### Building that environment from inside a tool session

- **Run long installs DETACHED.** `uv sync`/`pip` resolve by spawning a child interpreter; inside the
  Hermes tool's process tree that child dies with `Caused by: No child processes (os error 10)` — the
  same quirk behind flaky `git push` in these sessions — so the install fails instantly with a message
  that reads like a broken Python. Use
  `tmux new-session -d -s <task> '<cmd>; echo EXIT=$? > ~/.hermes/cache/scratch/<task>.log'` and poll
  the log (tmux is the detach that works from these sessions). Full build + verification procedure:
  skill `hermes-runtime-maintenance`.
- **`uv venv` ships no pip, and it can fail to pick an interpreter.** `Failed to discover managed
  Python installations / Could not detect either glibc version nor musl libc` is interpreter
  discovery, not Python. Pass the binary explicitly
  (`uv venv --python /home/habib/.local/bin/python3.13 <dir>`) or add `--seed` when pip is needed.
- **Verify an environment by USING it**, not by the installer's exit code: `<env>/bin/python -V`, the
  package count against the known-good env, and an import of the core plus compiled deps (`pytest`,
  the DB driver, `cryptography`). A green sync only means it resolved; a skipped native extra is what
  reappears as collection errors.

## 🔴 A DB-backed suite with no database behind it lies about the code (Sep 2026)

Both A2Square repos ship `docker-compose.test.yml` with **different contracts** —
`portal_backend_lambda_eventbridge` = `test_user`/`test_db` on host **:5433** (postgres:17),
`tapease_portal_fastapi_a2square` = `postgres`/`test_db` on host **:5432** (postgres:15) —
and `a2square_weekly_test_runner.py` had NO database handling at all. Result: `tests/database/`
and `tests/middleware/` raised `EEE…` at 1% and the process then **wedged** (0% CPU, no further
output) on connection attempts, so the leg reported failures that said nothing about the code.
A `test_user@5433` URL hardcoded for one repo is wrong for the other — **derive the contract from
that repo's own compose file**:

```python
def _compose_contract(text):
    """(service, user, password, db, host_port) — handles both `- KEY=val` and `KEY: val` forms."""
    grab = lambda k: (re.search(rf'{k}\s*[:=]\s*["\']?([^"\'\s#]+)', text) or [None, None])[1]
    svc = re.search(r'services:\s*\n\s+([\w.-]+):', text)
    port = re.search(r'["\'](\d+):5432["\']', text)
    return (svc.group(1) if svc else "postgres", grab("POSTGRES_USER") or "postgres",
            grab("POSTGRES_PASSWORD") or "postgres", grab("POSTGRES_DB") or "test_db",
            port.group(1) if port else "5432")
```

Bring it up before pytest and pass `env={**os.environ, "DATABASE_URL": url}`; wait for readiness with
`docker compose … exec -T <service> pg_isready -U <user>` (poll ~60s); run `python -m alembic upgrade head`
when `alembic.ini` exists; `compose down -v` afterwards. Symptom that proves a database was missing
rather than code being broken: **many `E` (errors, not failures) in one ordered block, then the process
sitting at 0% CPU forever.**

## Interpreters and timeouts must be chosen by TESTING, not by assumption (Sep 2026)

Two bugs in `a2square_weekly_test_runner.py` produced the same useless output for weeks:

- **`if venv_pytest.exists()`** decided the pytest interpreter. A venv created on Windows has a
  `.venv/bin/pytest` whose shebang points at a Windows path, so it exists and cannot execute. Probe
  each candidate with `<cmd> --version` (short timeout) and take the first that exits 0; chain repo
  venv → managed env in `~/.hermes/cache/testenvs/<repo>` → `sys.executable`.
- **`timeout=120` on a 2,558-test suite.** The reported error was a truncated
  `Command '['…/.venv/bin/pytest', 'tests/', '-q', …` — that text is EITHER a bad shebang (ENOENT) OR
  `TimeoutExpired`, so read the full untruncated error before "fixing" the venv. Size the budget to the
  suite (1800s here; the AML backend needs 1858s).
- **`NameError: name 'ensure_test_db' is not defined` — the entrypoint sat ABOVE its helpers.** In
  `a2square_weekly_test_runner.py` the `if __name__ == "__main__": sys.exit(main())` block was at line
  238 while `ensure_test_db()`/`stop_test_db()` were defined at lines 286/333 — so *every* run died the
  moment a pytest leg reached the DB bring-up, because Python only binds module-level names as it
  executes and the call under the entrypoint cannot see a `def` that appears later in the file. **Keep
  the entrypoint LAST.** After any script surgery, prove the helpers resolve *before* firing the job:
  `python -c "import importlib.util,sys; spec=importlib.util.spec_from_file_location('m',PATH); m=importlib.util.module_from_spec(spec); spec.loader.exec_module(m); print(callable(m.ensure_test_db))"`.
- **Peel the failure layers — expect each fix to expose the next one.** This job reported the same
  shape for weeks ("1 errors" / "timed out after 120 seconds") and every distinct cause sat BEHIND the
  previous one: the Windows-shebang venv hid the missing interpreter probe, the 120s cap hid the real
  suite runtime, and both hid the NameError above. **After fixing one layer, re-run the job before
...
**…but discriminate flake from defect with the escalation state, never with retries.**
`~/.hermes/state/amlhive_known_flaky.json` carries `{"<suite>": {"consecutive": N, "last": …}}`
per suite. The whitelist tolerates a matching failure only for its first two runs and escalates on the
third, so **a count far above 3 means a byte-identical failure has been re-escalated for days — that
is a REAL defect, not flake.** Treat "passes solo ⇒ flaky" as applying only to an intermittent red:
a state-specific deep scan that is red on *every* run while the default scan passes is the classic
real-defect shape, because the violation lives in a colour state nothing else exercises. Never make
such a run green by enabling `retries` or raising a timeout — that converts a real defect into a
silent one (`--retries=0` in a reproduction is deliberate). Note also that nothing acts on an
escalation — the runner detects, escalates and calls the diagnosis route but opens no issue, PR or
nudge, so assume nobody was ever told; the diagnosis itself is capped (~120 s), which is a statement
about the cap and not about the difficulty of the bug.

**Resolve an axe colour-contrast failure numerically, not by eye.** Pull the custom properties from
`frontend/app/landing.css` and compute WCAG ratios in python — linearise each channel (`c/12.92`
below 0.04045, else `((c+0.055)/1.055)**2.4`), `L = 0.2126R + 0.7152G + 0.0722B`, ratio
`(Lmax+0.05)/(Lmin+0.05)` — then test each foreground token against the background it ACTUALLY lands
on (a selected and an unselected state are different backgrounds):

- A near-white text token on a **white** surface is the classic state-specific failure (~1.1:1, a hard
  fail) and it appears only in the state that flips a surface to white — check every token that can
  land there, not just the one axe names first.
- State-tracking colours can each pass while an unrelated token fails: the badge/toggle pair a spec's
  own comment discusses is often fine, and the violation is a third token.
- A dark-accent token on a dark card can still fail (~3.6–4.4:1) — counts only when measured against
  the real background, never assumed from the token's name.

**Reproduce in a separate worktree.** The runner force-syncs `~/code/amlhive1` (fetch → checkout
`pluto_pr` → `reset --hard` → merge `origin/dev`), so a reproduction started there can be reset out
from under you mid-investigation; use a sibling worktree of the same repo and leave the runner's
branch untouched.
...
- `references/isolated-worktree-scan-flow.md` — isolated pluto_pr worktree procedure for advisory scans (CRAP weekly): setup, per-run BLOCKED gates, no reset --hard on the shared checkout, worktree-log append + coverage-drift pitfalls (Sep 2026)
...
|---------|---------------|--------|
| **AMLHive backend** | auto-discovered via `get_instance_id("amlhive-prod-backend", "i-04f81ec595a004caf")` (fallback refreshed Sep 12, 2026) | Name tag |
...
- **Docker Desktop unreachable from WSL → enable WSL integration; here is the exact switch.** The CLI is on PATH but `/var/run/docker.sock` is missing, so every Docker-dependent leg fails. The setting lives in `%APPDATA%\Docker\settings-store.json` (`/mnt/c/Users/<user>/AppData/Roaming/Docker/settings-store.json`) as `"EnableIntegrationWithDefaultWslDistro": false`; Docker Desktop's Settings → Resources → WSL Integration drives that same key. Back the file up, set it `true`, then restart the engine (`docker desktop restart` when the CLI supports it, else stop/start `Docker Desktop.exe`). `/var/run/docker.sock` then appears and `docker ps` works from inside WSL. The restart stops running containers — check the blast radius FIRST with the Windows binary, which reaches the daemon even while integration is off: `"/mnt/c/Program Files/Docker/Docker/resources/bin/docker.exe" ps` (all containers came back on this host after the restart). Discriminator: `/mnt/wsl/docker-desktop` can exist while the socket does not — the mount alone is NOT integration; test with `docker info`, not with `ls /mnt/wsl`.
...
- **A2Square "0/31" was a FALSE GREEN, not a test fail (root-caused 01 Sep 2026).** The 4 Mondays of `0/31` were 31 pytest COLLECTION errors (build/deps smell — `pytest-timeout` missing etc.) that the runner reported as ✅: it only counted `failed` in the exit code and never counted `errors`, and there was no no-tests guard. Fixed in `a2square_weekly_test_runner.py` (01 Sep): (a) no-tests guard — no `passed/failed/error` markers + non-zero exit → `errors=1`; (b) exit code and OVERALL line now include `errors` (`overall_failures += failed + errors`); (c) diagnosis routes on `errors` too. Lesson: a runner that parses `N error` must treat errors as failures in its EXIT code, or a broken environment looks green for weeks. Mirror this guard in ANY multi-repo runner.

...
## Open-Source Music Models: HeartMuLa

For generating music locally (rather than via Suno's API), HeartMuLa is an Apache-2.0 music generation model that produces full songs from lyrics + tags. See `references/heartmula.md` for installation, dependency fixes, and usage.

## Open-Source Music Generation: HeartMuLa

[HeartMuLa](https://github.com/HeartMuLa/heartlib) is an open-source alternative to Suno that generates music from lyrics + tags, with multilingual support. It includes HeartCodec (12.5Hz music codec) and supports models up to 7B.

**Hardware requirements:** 8GB+ VRAM minimum (3B model peaks at ~6.2GB with `--lazy_load`), 16GB+ recommended.
...
# Then apply dependency patches (see references/heartmula.md)
# Download checkpoints to ./ckpt/
...
**Lyrics format:** use `[Intro]`, `[Verse]`, `[Chorus]`, `[Bridge]`, `[Outro]` structural tags.

**Key parameters:** `--max_audio_length_ms 240000` (4 min max), `--temperature 1.0`, `--cfg_scale 1.5`.

**Critical patching:** Before first run, apply two patches to the source code:
...
See `references/heartmula.md` for the full installation guide, dependency compatibility fixes, and troubleshooting.

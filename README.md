...
| **[](./skills/tapease-db-access/SKILL.md)** | How to connect to a Tap-Ease Postgres — which local port is the dockerised local DB, which is an SSM tunnel to dev/staging, which is PRODUCTION — plus the psql/asyncpg credential workaround and the naive-UTC datetime convention. Use for "check the dev database", "run this query on staging/prod", "update my dev database", or a blocked/misrouted DB connection. | - |

...
- [Ensemble Self-Review Before Submission](#ensemble-self-review-before-submission)
...
**Worked example** (point-by-point format for an actual post-submission rebuttal):
```
...
- Use `latexdiff` to generate a marked-up PDF showing changes (see [submission-mechanics.md](submission-mechanics.md) for LaTeX tooling)
...
## Ensemble Self-Review Before Submission

Beyond the self-checklist above, simulate the actual review process before submission with an **ensemble reviewing + meta-reviewer** pattern. The key insight from automated research pipelines (notably SakanaAI's AI-Scientist): ensemble reviewing with a meta-reviewer produces far more calibrated feedback than a single review pass.

...
description: Pluto's redesigned morning briefing — action-first, scannable, self-improving. Uses briefing_improver.py engine. The "Option C" briefing that wows. Use when running, debugging, or extending the daily morning briefing pipeline, adjusting its format or signal sources, or handling the Saturday/Sunday weekend briefing variants.
...
- `references/design-rationale.md` — why v2 exists and the Honcho-derived design principles behind the action-first, scannable format.

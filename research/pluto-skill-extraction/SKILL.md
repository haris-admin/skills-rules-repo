...
- `references/operational-tracker.md` — running tracker of persistent operational issues and pending skill proposals across daily extraction cycles.
- `references/skill-extractor-brokenpipe-fix.md` — the fix for `BrokenPipeError` crashes in `skill_extractor.py` when the cron scheduler closes stdout early.
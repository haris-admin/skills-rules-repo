...
- `references/env-credential-pattern.md` — the .env credential-reading pattern (read secrets from `.env` at runtime instead of hardcoding them in scripts).
- `references/imaplib-pattern.md` — the working `imaplib` Gmail extraction pattern used in place of the broken himalaya `auth.cmd` approach.
